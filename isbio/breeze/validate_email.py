...
def get_mx_ip(hostname):
	if hostname not in MX_DNS_CACHE:
		try:
			...
			logger.info('MX records of ' + str(hostname) + ' : ' + str(MX_DNS_CACHE[hostname]))
		except ServerError as e:
			if e.rcode == 3:  # NXDOMAIN (Non-Existent Domain)
				logger.error(str(hostname) + ' is not an existing domain')
...
def validate_email(email, check_mx=False, verify=False, debug=False, smtp_timeout=10, fast=False):
